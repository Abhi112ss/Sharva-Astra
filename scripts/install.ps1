...
$RepoUrlSsh = "git@github.com:Abhi112ss/Sharva-Astra.git"
$RepoUrlHttps = "https://github.com/Abhi112ss/Sharva-Astra.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Sharva-Astra Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by TCS.                        |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/Abhi112ss/Sharva-Astra/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/Abhi112ss/Sharva-Astra/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/Abhi112ss/Sharva-Astra/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
function Start-GatewayIfConfigured {
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: sharva gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: sharva gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host ""
    Write-Host "   sharva              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   sharva setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   sharva config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   sharva config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   sharva gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   sharva update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/Abhi112ss/Sharva-Astra/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
